<table border=1 width=100% cellpadding=3 cellspacing=0>
    <tr>
        ...
        <th rowspan="2">HP</th>
        <th rowspan="2">PINJAMAN BOTOL</th>
        <th rowspan="2">PINJAMAN KRAT</th>
    ...
    @foreach ($data as $item)
        <tr>
          ...
          <td>{{ $item->pelanggan->total_pinjaman_botol ?: 0 }}</td>
          <td>{{ $item->pelanggan->total_pinjaman_krat ?: 0 }}</td>
          @foreach ($produk as $p)
            @php 
            $itemProduk = array_filter($item->item->toArray(), function ($i) use ($p) {
              return $i->barang_id == $p->id;
            }, 0);
            $itemProduk = array_values($itemProduk);
            @endphp

            <td>{{ $itemProduk ? @$itemProduk[0]->jumlah : 0 }}</td>
          @endforeach
